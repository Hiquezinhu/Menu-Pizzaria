...
const sacola: { item: string; preco: number }[] = [];
...
// ========== SUBMENU PIZZAS ==========
function menuPizzas(): void {
  const pizzas: { [key: string]: { nome: string; preco: number } } = {
...
function menuRefrigerantes(): void {
  const bebidas: { [key: string]: { nome: string; preco: number } } = {
...
// ========== SACOLA ==========
function visualizarSacola(): void {
  console.log("\n===| Sacola de Compras |===");
  if (sacola.length === 0) {
...
function finalizarPedido(): void {
  ...
  rl.question("Possui cupom de desconto? (Digite ou pressione Enter): ", (cupom) => {
    ...
    rl.question("Escolha a forma de pagamento: ", (pagamento) => {
      ...
      const pedidoFinal =
        sacola.map((i) => `${i.item} - ${i.preco}$`).join("\n") +
